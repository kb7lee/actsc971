...
def samplepath(S10,S20,sigma1,sigma2,r,T,M,N,rho):
    deltat=T/(N-1)
    S1=np.zeros((M,N))
    S1=mat(S1)
    S2=np.zeros((M,N))
...
def put_calculation(S10,S20,K,sigma1,sigma2,r,T,M,N,rho):
    deltat=T/(N-1)
    discount=np.exp(-r*deltat)
    [S1,S2]=samplepath(S10,S20,sigma1,sigma2,r,T,M,N,rho)
...
def call_calculation(S10,S20,K,sigma1,sigma2,r,T,M,N,rho):
    deltat=T/(N-1)
    discount=np.exp(-r*deltat)
    [S1,S2]=samplepath(S10,S20,sigma1,sigma2,r,T,M,N,rho)
    value=mat(np.zeros((M,N)))
...
for i in range(0,100):
    simulation[i,0]=put_calculation(40,40,40,0.2,0.2,r,1,M,N,rho)
np.mean(simulation)
np.std(simulation)